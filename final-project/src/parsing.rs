...
pub fn example(input_filename: &str, output_filename: &str)-> Result<(), Box<dyn Error>> {
    ...
    pub fn from_midi(input_filepath: &str) -> Result<(), Box<dyn Error>> {
        // Read the midi file
        let bytes = fs::read(input_filepath)?;
        let input_midi_file = Smf::parse(&bytes)?;
        // println!("{0:#?}", input_midi_file); // prints debug representation of midi file
