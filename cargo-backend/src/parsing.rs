...
pub fn to_midi(parsed_sequence: Vec<(Note, f32)>, output_filename: &str, metrical: u16) {
    ...
    let header = Header::new(Format::SingleTrack, metrical_timing); // create file header

    // initialize track vars
    let (mut predicted_track, mut time_since_note, mut note_histogram) =
        (Track::new(), 0.0, vec![0; 127]);
