...
fn run() -> Result<()> {
    ...
    let (num_octaves, lowest_allowed_pitch, quantized_durations, melody_pitch_dif, order) =
        (3, 50, vec![0.0625, 0.125, 0.25, 0.5, 1.0, 2.0, 4.0, 60.0], 36, 3); 
    ...
    for midi_file in genre_files {
        ...
        // push sequence and metrical to vecs
    ...
    let chain = markov::train_model(&genre_sequence, order)?;
    ...
    parsing::to_midi(parsed_sequence, filename, avg_metrical); // create and save a midi file with name filename
